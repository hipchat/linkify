package com.hipchat {
	...
	public class Link {
		...
		public static function linkify(text:String, 
			...
			text = Link.matchAndReplace(RE_EMAIL_PATTERN, text, false, false, dispatchEvent, truncateLength, matchedLinks);
			text = Link.matchAndReplace(RE_FULL_URL, text, true, false, dispatchEvent, truncateLength, matchedLinks);
			text = Link.matchAndReplace(RE_OTHER_URL, text, true, true, dispatchEvent, truncateLength, matchedLinks);
			
			if (emoticonify)
				text = Link.emoticonText(text);
